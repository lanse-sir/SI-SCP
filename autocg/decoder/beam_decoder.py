...
from autocg.utils.tensor_ops import get_long_tensor
from autocg.utils.tensor_ops import get_tensor
...
class TopKDecoder(nn.Module):
    ...
    def __init__(self, decoder_rnn, k):
        ...
        # self.vocab = vocab
        # self.id2word = {idx: word for word, idx in self.V.items()}

    # def id2word(self, idx):
    #     id2word = {idx: word for word, idx in self.vocab.items()}
    #     return id2word.get(idx, 'unk')

    def beam_search(self, encoder_hidden, encoder_outputs, decode_max_time_step,
                    sent_type_emb=None, beam_size=5, sent_mask=None, template_mask=None):
        self.k = beam_size

        metadata = self.forward(
            inputs=None,
    ...
    def forward(self, inputs=None, encoder_hidden=None, encoder_outputs=None,
        ...
        idx_perm = get_long_tensor([i + batch_size * j for i in range(batch_size) for j in range(self.k)])
        ...
        if self.rnn.use_attention:
            ...
            inflated_encoder_outputs = inflated_encoder_outputs[idx_perm, :, :]
            # inflated_temp_encoder_outputs = _inflate(temp_encoder_outputs, self.k, 0)
            # if sent_mask is not None and template_mask is not None:
            # inflated_sent_mask = _inflate(sent_mask, self.k, 0)[idx_perm, :]
            inflated_template_mask = _inflate(template_mask, self.k, 0)[idx_perm, :]

            # inflated_temp_encoder_outputs = inflated_temp_encoder_outputs[idx_perm, :, :]
            # self.rnn.attention.set_mask(inflated_sent_mask, inflated_template_mask)
            self.rnn.attention.set_mask(inflated_template_mask)
        else:
            inflated_encoder_outputs = None
            inflated_temp_encoder_outputs = None
            # inflated_sent_mask = None
            inflated_template_mask = None
        ...
        # hypotheses = [[[[], 0.0] for j in range(self.k)] for i in range(batch_size)]
        if sent_type_emb is not None:
            sent_type_emb = sent_type_emb.repeat(self.k, 1, 1)[idx_perm, :, :]
        else:
            sent_type_emb = None
        hidden = hidden[:, idx_perm, :]

        for _ in range(0, max_length):

            # Run the RNN one step forward
            log_softmax_output, hidden, _, _ = self.rnn.forward_step(input_var, hidden,
                                                                     inflated_encoder_outputs,
                                                                     function=function,
                                                                     temp_mask=inflated_template_mask,
                                                                     sent_type_tensor=sent_type_emb)
            # _ = log_softmax_output.max(dim=2)[1]
            ...
            # log_softmax_output = log_softmax_output[idx_perm]
            ...
            #         hypotheses[i][j][0].append(self.id2word(candidates[i, j].item()))
            #         score = scores[i, j].data.item()
            #         hypotheses[i][j][1] += score
        ...
        metadata['h_t'] = h_t
        metadata['dec_outputs'] = decoder_outputs
        metadata['dec_hidden'] = decoder_hidden
        metadata['score'] = [l[0] for l in s]
    ...
    def _backtrack(self, nw_output, nw_hidden, predecessors, symbols, scores, b, hidden_size):
        ...
        ret_p = torch.stack(ret_p, dim=2).squeeze(3)

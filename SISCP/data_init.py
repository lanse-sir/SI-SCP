...
def batch_serialization_parallel(batch, word_vocab, parse_vocab, opt, pos_vocab=None, if_train=True):
...
def batch_tree_encoding(batch, word_vocab, parse_vocab, opt, pos_vocab=None, parent_child=None, if_train=True, node_noise=False):
    batch_size = len(batch)
    ...
    def dfs_travel(nodes, tree, root, stack, lj_matrix, leave_node, level, height):
        ...
        if len(tree[root]) != 0:
            for child in tree[root]:
                # lj_matrix.append((stack[-1], child))
                lj_matrix.append((root, child))
    ...
    def dfs_travel_sampling(nodes, tree, root, stack, lj_matrix, leave_node, leave_node_drop, level, cnt, height):
        nodes.append(root)
        level.append(height)
        ...
        if len(tree[root]) != 0:
            for child in tree[root]:
                # lj_matrix.append((stack[-1], child))
                if node_noise and parent_child is not None and height == 3 and random.random() < opt.node_drop:  #spcify different height.
                    parent = root.rsplit('-', maxsplit=1)[0]
                    process_child = child.rsplit('-', maxsplit=1)[0]
                    if parent in parent_child:
                        non_overlap = parent_child[parent]
                    else:
                        # otherwise, set to empty set.
                        non_overlap = []
                    
                    if process_child in non_overlap:
                        non_overlap.remove(process_child)

                    sub_child = tree[child]
                    if len(non_overlap) > 0:
                        child = random.choice(non_overlap)
                        child = child + "-substitute" + str(cnt["substitute"])
                        cnt["substitute"] += 1
                        tree[child] = sub_child
                
                # path attention. 
                for n in stack:
                    lj_matrix.append((n, child))
                
                lj_matrix.append((root, child))
                dfs_travel_sampling(nodes, tree, child, stack, lj_matrix, leave_node, leave_node_drop, level, cnt,
                                    height + 1)
            stack.pop()
        else:
            if "substitute" in root:
                leave_node_drop.append(len(leave_node))
            leave_node.append(len(nodes) - 1)
            stack.pop()
    ...
    def deep_travel_node(parses):
        ...
        for parse in parses:
            ...
            # dfs_travel_sampling(node, parse, 'ROOT', stack, lj, leave_node, leave_node_drop, level, cnt, height=1)
            nodes.append(node)
            ljs.append(lj)
            leave_nodes.append(leave_node)
            leave_nodes_drop.append(leave_node_drop)
            levels.append(level)
        return nodes, levels, ljs, leave_nodes, leave_nodes_drop
    
    def sibling_posi_mask(levels, max_node_len):
        sibling_posi = []
        ...
        for level in levels:
            ...
            for item in level:
                ...
                posi.append(cnt[item])
            sibling_posi.append(posi)
        
        tgt_sibling_attn_mask = torch.zeros(batch_size,max_node_len, max_node_len).byte()
        for batch,level in enumerate(levels):
            for i in range(len(level)):
                for j in range(len(level)):
                    if level[j]==level[i]:
                        tgt_sibling_attn_mask[batch, i, j]=1
        
        return sibling_posi, tgt_sibling_attn_mask
    
    # parse tree process.
    if type(batch[0][1]) is not dict:
        parsers = [json.loads(pair[1]) for pair in batch]
    else:
        parsers = [pair[1] for pair in batch]
    ...
    #print(tgt_trees[0])
    nodes, levels, ljs, leave_nodes, leave_nodes_drop = deep_travel_node(tgt_trees)
    
    #print(nodes[0])
    max_node_seq_len = max(list(map(len, nodes)))
    
    # represent sibling relation.
    sibling_posi, tgt_sibling_attn_mask = sibling_posi_mask(levels, max_node_seq_len)
    
    # represent parent-child relation 
    tgt_trees_attn_mask = torch.eye(max_node_seq_len).byte()
    tgt_trees_attn_mask = tgt_trees_attn_mask.unsqueeze(0).repeat(batch_size, 1, 1)

    #tgt_trees_attn_mask = torch.zeros(batch_size, max_node_seq_len, max_node_seq_len).byte()

    for idx, lj in enumerate(ljs):
        for head, tail in lj:
            row, column = nodes[idx].index(head), nodes[idx].index(tail)
            #tgt_trees_attn_mask[idx, row, row] = 1
            #tgt_trees_attn_mask[idx, row, column] = 1
            tgt_trees_attn_mask[idx, column, row] = 1  
    #print(tgt_trees_attn_mask[0])
    tgt_nodes_seq = [data_to_idx([n.rsplit('-', maxsplit=1)[0] for n in seq], parse_vocab) for seq in nodes]
    #print(tgt_nodes_seq[0])
    #exit()
    ...
    # sibling position.
    _, sibling_posi_tensor, _ = padding(sibling_posi)
    
    leaf = [[nodes[i][j] for j in leave_nodes[i]] for i in range(batch_size)]
    ...
    if if_train:
        ...
        for i in range(batch_size):
            ...
            for j in range(min(len(tgt_idx_seq[i]), opt.sent_max_time_step)):
                ...
                # leave drop mask...
                if flag - 1 in leave_nodes_drop[i]:
                    drop_mask.append(0)
                else:
                    drop_mask.append(1)
            att_targets.append(t)
        ...
        _, drop_masks_tensor, _ = padding(drop_masks)
    else:
        tgt_seq_tensor = torch.Tensor([0.])
        inp_seq_tensor = torch.Tensor([0.])
        inp_mask = torch.Tensor([0.])
        smooth_one_hot = torch.Tensor([0.])
        tgt_mask = torch.Tensor([0.])
        drop_masks_tensor = torch.Tensor([0.])
    ...
    if torch.cuda.is_available():
        return {
            ...
            'sibling_posi': sibling_posi_tensor.cuda(),
            'sibling_mask': tgt_sibling_attn_mask.cuda(),
            ...
            'attn_label': smooth_one_hot.cuda(),
            'tgt_mask': tgt_mask.cuda(),
            'drop_mask': drop_masks_tensor.cuda(),
            'nodes': nodes,
            'leaf': leaf
        }
    else:
        return {
            ...
            'sibling_posi': sibling_posi_tensor,
            'sibling_mask': tgt_sibling_attn_mask,
...
def remove_leaves_from_trees(trees, bpe=False, pos=False):
    for tree in trees:
        #print(tree)
